...
async def crawl_ppomppu(page, keyword: str) -> list[dict]:
    ...
    try:
        for kw in COMMUNITY_KEYWORDS:
            ...
            print(f"    [뽐뿌] '{kw}' 링크 {len(links)}개 발견")
            ...
            for link in links:
                try:
                    ...
                    if not any(k in title_clean for k in COMMUNITY_KEYWORDS):
                        continue
...
부정적인 내용(불만, 오류, 환불, 안됨, 느림, 최악, 사기, 문제 등)의 글 인덱스를 반환해주세요.
...
{"negative_indices": [0기반 인덱스 번호들]}"""
        ...
        try:
            ...
            filtered[api_type] = items  # 오류 시 전체 포함
...
아래는 이번 주 커뮤니티에서 수집된 '아이즈모바일' 관련 게시글 및 네이버 부정 언급 데이터입니다.

기간: {week_label}
총 커뮤니티 수집: {len(all_posts)}건
...
def build_naver_table(naver_negative: dict) -> str:
    type_names = {"blog": "블로그", "cafe": "카페"}
    html = ""
    for api_type, items in naver_negative.items():
        if not items:
...
<table style="width:100%; border-collapse:collapse; font-size:12px;">
  <thead>
    <tr style="background:#f8f8f8;">
      <th style="text-align:left; padding:7px 10px; border-bottom:1px solid #e0e0e0; width:50%;">제목</th>
...
def send_email(subject: str, report: str, all_posts: list[dict], naver_negative: dict):
    ...
    naver_html       = build_naver_table(naver_negative)
    naver_total      = sum(len(v) for v in naver_negative.values())
    community_total  = len(all_posts)
...
<body style="font-family:sans-serif; line-height:1.7; color:#333; max-width:720px; margin:0 auto; padding:24px;">
  ...
  <p style="font-size:12px; color:#999; margin-bottom:20px;">
    커뮤니티 {community_total}건 · 네이버 부정글 {naver_total}건 수집
  </p>
  ...
  <h2 style="font-size:16px; font-weight:500; margin-top:36px; margin-bottom:4px;
    border-bottom:1px solid #eee; padding-bottom:10px;">커뮤니티 수집 게시글</h2>
  {community_html}

  {('<h2 style="font-size:16px; font-weight:500; margin-top:36px; margin-bottom:4px; border-bottom:1px solid #eee; padding-bottom:10px;">네이버 부정 언급 목록</h2>' + naver_html) if naver_html else ""}
...
async def main():
    ...
    unique_posts = []
    for post in all_posts:
        url_clean = post["url"].split("?")[0] if post["url"] else post["title"]
    ...
    for kw in NAVER_KEYWORDS:
        result = await crawl_naver(kw)
        for api_type in naver_data:
            naver_data[api_type].extend(result[api_type])
